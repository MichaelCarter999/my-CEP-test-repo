...
# ✅ Template mapping
ROLE_TO_TEMPLATE = {
    "core": "Network Generic Device by SNMP",
    "distribution": "Network Generic Device by SNMP",
    "access": "Network Generic Device by SNMP",
    "spur": "Network Generic Device by SNMP",
}
...
def group_id(name: str) -> str:
    ...
    return found[0]["groupid"] if found else call(
...
def get_template_id(name: str) -> str:
    t = call("template.get", {"filter": {"host": [name]}})
    if not t:
...
def main():
    ...
    for d in model["devices"]:
        ...
        # -------------------------
        # Templates (FINAL LOGIC)
        # -------------------------
        templates = []

        if "snmp" in d["monitoring"]:
            template_name = ROLE_TO_TEMPLATE[d["role"]]
